...
class LibraryManager:
    ...
    def filter_by_read_status(self, status):
        """Filter books by read status"""
        if status == "All Books":
            return self.library
        elif status == "Read":
            return [book for book in self.library if book["read"]]
        else:  # Unread
            return [book for book in self.library if not book["read"]]
...
def main():
    ...
    with tab4:
        ...
        if stats["total"] > 0:
            ...
            # Create pie chart
            fig, ax = plt.subplots()
            ax.pie(pie_data['Count'], labels=pie_data['Status'], autopct='%1.1f%%', 
                   colors=['#4CAF50', '#FF5722'], explode=[0.05, 0])
            ax.set_title('Read vs Unread Books')
            st.pyplot(fig)
            ...
            chart = alt.Chart(genre_data).mark_bar().encode(
                ...
                height=min(300, len(genre_counts) * 40)
            )
